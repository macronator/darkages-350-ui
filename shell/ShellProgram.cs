...
internal static class ShellProgram
{
    ...
    //   da350-shell <DarkAges.dat> <ui-350.json> --shot out.png [--hp N] [--mp N] [--open a,b]
    //                                                       render one frame headless (no window) and exit
    [STAThread]
    private static int Main(string[] args)
    {
        if (args.Length < 2)
        {
            Console.WriteLine("usage: da350-shell <DarkAges.dat> <ui-350.json> [--shot out.png] [--hp N] [--mp N] [--open a,b]");
            return 1;
        }

        var dat = new DatArchive(args[0]);
        var spec = UiSpec.Load(args[1]);
